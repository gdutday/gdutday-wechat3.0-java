...
docker build -t registry.gdutelc.com/gdutday/gdutday:"$VERSION" .
...
# 给镜像加上 latest 标签
docker tag registry.gdutelc.com/gdutday/gdutday:"$VERSION" registry.gdutelc.com/gdutday/gdutday:latest

# 登录 Docker 注册表（请替换为实际的用户名、密码和注册表地址）
# echo "your-username" | docker login -u "your-username" --password-stdin registry.example.com

# 将镜像推送到 Docker 注册表
docker push registry.gdutelc.com/gdutday/gdutday:"$VERSION"
docker push registry.gdutelc.com/gdutday/gdutday:latest
